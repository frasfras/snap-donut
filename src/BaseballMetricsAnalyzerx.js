...
const BaseballMetricsAnalyzerx = () => {
    ...
    if (videoRef.current && canvasRef.current && !processingRef.current) {
      ...
      try {
        ...
        const response = await fetch('http://localhost:5000/process-frame', {
          ...
          body: JSON.stringify({
            frame: frameData,
            fps: videoRef.current.playbackRate * 30, // Approximate FPS
          }),
        });
        ...
        if (result.position && result.velocity) {
          // Draw ball position
          ctx.beginPath();
          ctx.arc(result.position[0], result.position[1], 5, 0, 2 * Math.PI);
          ctx.fillStyle = 'red';
          ctx.fill();
          ...
          <div className="relative aspect-video bg-gray-100 rounded-lg overflow-hidden">
            {videoUrl ? (
              ...
              <div className="flex items-center justify-center h-full text-gray-500">
                Upload a baseball video to analyze
              </div>
            )}
          </div>
...
export default BaseballMetricsAnalyzerx;